...
- Beyond regular services for making HTTP requests, there are also services for handling your authentication token (Spring Security), passing this token into backend requests, and errors.
  - The [token service](ng-frontend/src/app/services/token.service.ts) stores JSON web tokens generated by the backend, as well as checks its validity (in terms of expiry time).
...
- Even while you are authenticated, the backend checks for these for security purposes:
  ...
  - A resource that you are trying to access must belong to you. These checks are done in [Service](sb-backend/src/main/java/ibf2022/tfipminiproject/services) classes.
...
- Using these `UUIDs` as necessary, free to make API calls to the following protected endpoints to test them:
  - Do not do `PUT` requests; these are unimplemented and untested.
